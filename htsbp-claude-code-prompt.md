...
│   ├── CONTRIBUTING.md
│   └── workflows/
│       ├── collect.yml                 # 日次収集 → JSONファイル更新 → git push → Netlify再デプロイ
...
[
  ...
  },
  {
    ...
    "url": "https://github.com/YOUR_USERNAME/htsbp/issues",
    "type": "community",
...
```typescript
// 実装要件:
// 1. Anthropic Messages API を直接呼び出し（model: claude-sonnet-4-20250514）
...
```
ANTHROPIC_API_KEY=              # OpenClaw / Claude API用
NOTIFICATION_WEBHOOK_URL=       # エラー通知 Webhook（LINE/Slack/Discord）
```
...
LP / docs.html の「Report a threat」リンクはこのIssueテンプレートへのURL（`https://github.com/YOUR_USERNAME/htsbp/issues/new?template=new-threat.yml`）に向ける。
...
git branch -M main
git remote add origin https://github.com/YOUR_USERNAME/htsbp.git
...
「Daily IDPI Collection」を手動実行 → data/ が更新されcommitされることを確認
「Health Check」を手動実行 → 成功することを確認
```
...
□ Netlify Analytics でアクセスデータが見える
□ GitHub Actions の cron がスケジュールされている
□ GitHub README が正しく表示される
□ data/ ディレクトリに11ドメインのJSONファイルがある

全て完了 → 完全自動運用開始。以降、人間がやることは:
- Netlify Analytics を週1回確認
- Webhook通知が来たら内容確認
- GitHub Issues（new-threat）を定期チェック → 確認後にデータ追加PRを作成
```
