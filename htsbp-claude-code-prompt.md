...
**登録対象（どちらか一方を満たせばよい）：**
1. HTMLにIDPIペイロードが仕込まれたサイト（直接検出）
2. 信頼できる情報ソース（Unit42等）の記事に「AIエージェント経由のプロンプトインジェクション」として明記されたサイト（一時的にペイロードが消えていても登録可）

**登録対象外：**
- SEOポイズニングのみのサイト（AIエージェント経由のIDPIの言及なし）
- 人間ユーザーのみを標的にしたマルウェア配布・認証情報窃取
- 正規AIサービスのドメイン（攻撃被害者）
