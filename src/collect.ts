...
\`\`\``;
  ...
  if (!res.ok) {
    throw new Error(`Claude API ${src.id}: HTTP ${res.status}`);
  }
  ...
  try {
    ...
    console.warn(
      `[collect] ${src.id}: Claude 応答の JSON 解析失敗 (${msg}) — 0 件として継続。応答先頭: ${text.slice(0, 200)}`,
    );
    return [];
  }
}
...
async function reflectOne(c: Candidate, j: ComprehensiveJudgment): Promise<void> {
  ...
  try {
    ...
    // 失敗時も main に戻して次候補のワーキングツリーを汚さない
    try { execGit("git checkout main"); } catch { /* may already be on main */ }
  }
}
...
async function main(): Promise<void> {
  ...
  for (const s of sourceLog) {
    if (!s.ok) failures.push(`source ${s.id}: 巡回失敗`);
  }
  ...
  for (const c of candidates) {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.warn(`[collect] ${c.host} 失敗:`, msg);
      failures.push(`candidate ${c.host}: ${msg}`);
    }
  }

  if (failures.length > 0) {
    console.error(`[collect] 失敗 ${failures.length} 件 → exit 1:`);
